 ...
 */
function validateAndFormatPhone(input: string): {
	phone: string
	error?: string
} {
	...
	if (cleaned.startsWith('+39')) {
		...
		if (digits.length === 10) {
			return {
				phone: `+39 ${digits.substring(0, 3)} ${digits.substring(
					3,
					6
				)} ${digits.substring(6)}`,
			}
			...
			return {
				phone: `+39 3${digits.substring(0, 2)} ${digits.substring(
					2,
					5
				)} ${digits.substring(5)}`,
			}
			...
			return {
				phone: cleaned,
				error: `Итальянский номер должен содержать 10 цифр после +39 (найдено: ${digits.length})`,
			}
		}
		...
		return {
			phone: `+39 ${cleaned.substring(0, 3)} ${cleaned.substring(
				3,
				6
			)} ${cleaned.substring(6)}`,
		}
	...
	return {
		phone: input,
		error: 'Неверный формат телефона. Ожидается: +39 XXX XXX XXXX',
	}
}
...
function validateAndFormatName(input: string): {
	...
	if (input.length > 0) {
		...
		if (formatted !== input) {
			return {
				name: formatted,
				warning: `Имя автоматически исправлено: "${input}" → "${formatted}"`,
			}
		}
		return { name: input }
	}
